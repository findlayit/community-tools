...
namespace VS140 //Visual Studio 2015
{
    class Script
    {
        static string usage = "Usage: cscscript debugVS14.0 [/e] [[/prj] scriptFile]|[[/r] projectFile] ...\nLoads C# script file into temporary VS 10.0 C# project and opens it.\n\n" +
                                "</e> - Express edition of Visual Studio 2015\n" +
        ...
        static public void Main(string[] args)
        {
            ClearOldSolutions();
            //Debug.Assert(false);
            SetEnvironmentVariable("CSScriptDebugging", "VS14.0");
            ...
            else if (args[0].Trim().ToLower() == "/prj")
            {
                ...
                try
                {
                    VS140IDE.IsolateProject(scriptFile, Path.Combine(Path.GetDirectoryName(scriptFile), Path.GetFileNameWithoutExtension(scriptFile)));
                }
                catch (Exception e)
                {
            ...
            else if (args[0].Trim().ToLower() == "/r")
            {
                ...
                try
                {
                    VS140IDE.RefreshProject(projFile);
                }
                catch (Exception e)
                {
                    MessageBox.Show("Specified file could not be linked to the temp project:\n" + e.Message);
            ...
            else
            {
                try
                {
                    ...
                    string solutionFile = VS140IDE.CreateProject(scriptFile, tempDir);
                    ...
                    myProcess.StartInfo.FileName = VS140IDE.GetIDEFile(edition);

                    if (myProcess.StartInfo.FileName == "<not defined>")
                    {
                        if (edition == IDEEditions.express)
                            myProcess.StartInfo.FileName = VS140IDE.GetIDEFile(IDEEditions.normal);
                        else
                            myProcess.StartInfo.FileName = VS140IDE.GetIDEFile(IDEEditions.express);
                    }
                    ...
                    else
                    {
                        ...
                        //do not cleanup as VS2015 may relaunch the process so we cannot rely on myProcess.WaitForExit();
                        ClearOldSolutions();
                        return;

                        //do clean up
                        foreach (string file in VS140IDE.GetImportedScripts(projFile))
                        {
        ...
        static void AddToRecentScripts(string file)
        {
            try
            {
                ...
                string historyFile = Path.Combine(GetTempCSSDir(), "VS2014_recent.txt");
        ...
        public class VS140IDE
        {
            ...
            {
                string srcProjDir = @"Lib\Debug\VS14.0"; //relative to CSSCRIPT_DIR
                string scHomeDir = VS140IDE.GetEnvironmentVariable("CSSCRIPT_DIR");
                ...
                VS140IDE ide = new VS140IDE();

                ScriptParser parser = new ScriptParser(scriptFile, Script.SearchDirs, VS140IDE.isolating);
                ...
                if (VS140IDE.IsResxRequired(srcFile) && associatedXml == "")
                ...
                foreach (string file in importerdScripts)
                {
                    ...
                    XAML = srcFile.ToLower().EndsWith(".xaml");
                    if (!Path.GetFileName(srcFile).StartsWith("i_") && VS140IDE.IsResxRequired(srcFile) && associatedXml == "")
                    {
                        ide.InsertFile(srcFile, projFile, resxSrcFile, "");
                    }
                    else
                ...
                if (Environment.GetEnvironmentVariable("CSSCRIPT_VS_DROPASMINFO") != null && !VS140IDE.isolating)
            ...
            public void InsertReference(string asmFile, string projFile, string aliase = null)
            {
                ...
                if (VS140IDE.isolating || asmFile.IndexOf("assembly\\GAC") != -1 || asmFile.IndexOf("assembly/GAC") != -1)
            ...
            static public string[][] GetAvailableIDE()
            {
                ...
                {
                    name = "Open with VS2015E";
                    hint = "\t- Open with MS Visual Studio 2015 Express";
                    command = "\"" + scHomeDir + "\\csws.exe\" /c \"" + scHomeDir + "\\lib\\DebugVS14.0.cs\" /e \"%1\"";
                    retval.Add(new string[] { name, hint, command });
                }

                if (GetIDEFile(IDEEditions.normal) != "<not defined>")
                {
                    name = "Open with VS2015";
                    hint = "\t- Open with MS Visual Studio 2015";
                    command = "\"" + scHomeDir + "\\csws.exe\" /c \"" + scHomeDir + "\\lib\\DebugVS14.0.cs\" \"%1\"";
                    retval.Add(new string[] { name, hint, command });
                }
            ...
            static public string GetIDEFile(IDEEditions edition)
            {
                ...
                try
                {
                    string keyname = (edition == IDEEditions.express) ? @"WDExpress.cs.14.0\shell\Open\Command" : @"VisualStudio.cs.14.0\shell\Open\Command";
            ...
            {
                string[] content = VS140IDE.GetImportedScripts(projFile);
                ...
                RunPreScripts(scriptFile);
                string newSolution = VS140IDE.CreateProject(content[0], Path.GetDirectoryName(projFile));
                string[] newContent = VS140IDE.GetImportedScripts(Path.ChangeExtension(newSolution, ".csproj"));
