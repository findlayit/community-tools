...
namespace VS110
{
    class Script
    {
        static string usage = "Usage: cscscript debugVS11.0 [/e] [[/prj] scriptFile]|[[/r] projectFile] ...\nLoads C# script file into temporary VS 10.0 C# project and opens it.\n\n" +
        ...
        static public void Main(string[] args)
        {
            ClearOldSolutions();
            //Debug.Assert(false);
            SetEnvironmentVariable("CSScriptDebugging", "VS11.0");
            ...
            else if (args[0].Trim().ToLower() == "/prj")
            {
                ...
                try
                {
                    VS110IDE.IsolateProject(scriptFile, Path.Combine(Path.GetDirectoryName(scriptFile), Path.GetFileNameWithoutExtension(scriptFile)));
                }
                catch (Exception e)
                {
            ...
            else if (args[0].Trim().ToLower() == "/r")
            {
                ...
                try
                {
                    VS110IDE.RefreshProject(projFile);
                }
                catch (Exception e)
                {
                    MessageBox.Show("Specified file could not be linked to the temp project:\n" + e.Message);
            ...
            else
            {
                try
                {
                    ...
                    string solutionFile = VS110IDE.CreateProject(scriptFile, tempDir);
                    ...
                    myProcess.StartInfo.FileName = VS110IDE.GetIDEFile(edition);

                    if (myProcess.StartInfo.FileName == "<not defined>")
                    {
                        if (edition == IDEEditions.express)
                            myProcess.StartInfo.FileName = VS110IDE.GetIDEFile(IDEEditions.normal);
                        else
                            myProcess.StartInfo.FileName = VS110IDE.GetIDEFile(IDEEditions.express);
                    }
                    ...
                    else
                    {
                        ...
                        //do clean up
                        foreach (string file in VS110IDE.GetImportedScripts(projFile))
                        {
        ...
        static void AddToRecentScripts(string file)
        {
            try
            {
                ...
                string historyFile = Path.Combine(GetTempCSSDir(), "VS2012_recent.txt");
        ...
        public class VS110IDE
        {
            ...
            static private string CreateProject(string scriptFile, string tempDir, ProcessSourceFile fileHandler, bool copyLocalAsm)
            {
                ...
                string scHomeDir = VS110IDE.GetEnvironmentVariable("CSSCRIPT_DIR");
                ...
                VS110IDE ide = new VS110IDE();

                ScriptParser parser = new ScriptParser(scriptFile, Script.SearchDirs, VS110IDE.isolating);
                ...
                if (VS110IDE.IsResxRequired(srcFile) && associatedXml == "")
                ...
                foreach (string file in importerdScripts)
                {
                    ...
                    XAML = srcFile.ToLower().EndsWith(".xaml");
                    if (!Path.GetFileName(srcFile).StartsWith("i_") && VS110IDE.IsResxRequired(srcFile) && associatedXml == "")
                    {
                        ide.InsertFile(srcFile, projFile, resxSrcFile, "");
                    }
                    else
                ...
                if (Environment.GetEnvironmentVariable("CSSCRIPT_VS_DROPASMINFO") != null && !VS110IDE.isolating)
            ...
            public void InsertReference(string asmFile, string projFile, string aliase = null)
            {
                ...
                if (VS110IDE.isolating || asmFile.IndexOf("assembly\\GAC") != -1 || asmFile.IndexOf("assembly/GAC") != -1)
            ...
            static public string[][] GetAvailableIDE()
            {
                ...
                {
                    name = "Open with VS2012E";
                    hint = "\t- Open with MS Visual Studio 2012 Express";
                ...
                if (GetIDEFile(IDEEditions.normal) != "<not defined>")
                {
                    ...
                    hint = "\t- Open with MS Visual Studio 2012";
                    command = "\"" + scHomeDir + "\\csws.exe\" /c \"" + scHomeDir + "\\lib\\DebugVS11.0.cs\" \"%1\"";
                    retval.Add(new string[] { name, hint, command });
                }
            ...
            static public string GetIDEFile(IDEEditions edition)
            {
                ...
                try
                {
                    string keyname = (edition == IDEEditions.express) ? @"VCSExpress.cs.10.0\shell\Open\Command" : @"VisualStudio.cs.11.0\shell\Open\Command";
            ...
            {
                string[] content = VS110IDE.GetImportedScripts(projFile);
                ...
                RunPreScripts(scriptFile);
                string newSolution = VS110IDE.CreateProject(content[0], Path.GetDirectoryName(projFile));
                string[] newContent = VS110IDE.GetImportedScripts(Path.ChangeExtension(newSolution, ".csproj"));
