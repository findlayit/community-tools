...
namespace VS110 //Visual Studio 2013
{
    class Script
    {
        static string usage = "Usage: cscscript debugVS12.0 [/e] [[/prj] scriptFile]|[[/r] projectFile] ...\nLoads C# script file into temporary VS 10.0 C# project and opens it.\n\n" +
                                "</e> - Express edition of Visual Studio 2013\n" +
        ...
        static public void Main(string[] args)
        {
            ClearOldSolutions();
            //Debug.Assert(false);
            SetEnvironmentVariable("CSScriptDebugging", "VS12.0");
            ...
            else if (args[0].Trim().ToLower() == "/prj")
            {
                ...
                try
                {
                    VS120IDE.IsolateProject(scriptFile, Path.Combine(Path.GetDirectoryName(scriptFile), Path.GetFileNameWithoutExtension(scriptFile)));
                }
                catch (Exception e)
                {
            ...
            else if (args[0].Trim().ToLower() == "/r")
            {
                ...
                try
                {
                    VS120IDE.RefreshProject(projFile);
                }
                catch (Exception e)
                {
                    MessageBox.Show("Specified file could not be linked to the temp project:\n" + e.Message);
            ...
            else
            {
                try
                {
                    ...
                    string solutionFile = VS120IDE.CreateProject(scriptFile, tempDir);
                    ...
                    myProcess.StartInfo.FileName = VS120IDE.GetIDEFile(edition);

                    if (myProcess.StartInfo.FileName == "<not defined>")
                    {
                        if (edition == IDEEditions.express)
                            myProcess.StartInfo.FileName = VS120IDE.GetIDEFile(IDEEditions.normal);
                        else
                            myProcess.StartInfo.FileName = VS120IDE.GetIDEFile(IDEEditions.express);
                    }
                    ...
                    else
                    {
                        ...
                        //do not cleanup as VS2012 may relaunch the process so we cannot rely on myProcess.WaitForExit();
                        ClearOldSolutions();
                        return;

                        //do clean up
                        foreach (string file in VS120IDE.GetImportedScripts(projFile))
                        {
        ...
        static void AddToRecentScripts(string file)
        {
            try
            {
                ...
                string historyFile = Path.Combine(GetTempCSSDir(), "VS2013_recent.txt");
        ...
        public class VS120IDE
        {
            ...
            {
                string srcProjDir = @"Lib\Debug\VS11.0"; //relative to CSSCRIPT_DIR
                string scHomeDir = VS120IDE.GetEnvironmentVariable("CSSCRIPT_DIR");
                ...
                VS120IDE ide = new VS120IDE();

                ScriptParser parser = new ScriptParser(scriptFile, Script.SearchDirs, VS120IDE.isolating);
                ...
                if (VS120IDE.IsResxRequired(srcFile) && associatedXml == "")
                ...
                foreach (string file in importerdScripts)
                {
                    ...
                    XAML = srcFile.ToLower().EndsWith(".xaml");
                    if (!Path.GetFileName(srcFile).StartsWith("i_") && VS120IDE.IsResxRequired(srcFile) && associatedXml == "")
                    {
                        ide.InsertFile(srcFile, projFile, resxSrcFile, "");
                    }
                    else
                ...
                if (Environment.GetEnvironmentVariable("CSSCRIPT_VS_DROPASMINFO") != null && !VS120IDE.isolating)
            ...
            public void InsertReference(string asmFile, string projFile, string aliase = null)
            {
                ...
                if (VS120IDE.isolating || asmFile.IndexOf("assembly\\GAC") != -1 || asmFile.IndexOf("assembly/GAC") != -1)
            ...
            static public string[][] GetAvailableIDE()
            {
                ...
                {
                    name = "Open with VS2013E";
                    hint = "\t- Open with MS Visual Studio 2013 Express";
                    command = "\"" + scHomeDir + "\\csws.exe\" /c \"" + scHomeDir + "\\lib\\DebugVS11.0.cs\" /e \"%1\"";
                ...
                if (GetIDEFile(IDEEditions.normal) != "<not defined>")
                {
                    ...
                    hint = "\t- Open with MS Visual Studio 2013";
                    command = "\"" + scHomeDir + "\\csws.exe\" /c \"" + scHomeDir + "\\lib\\DebugVS12.0.cs\" \"%1\"";
                    retval.Add(new string[] { name, hint, command });
                }
            ...
            static public string GetIDEFile(IDEEditions edition)
            {
                ...
                try
                {
                    string keyname = (edition == IDEEditions.express) ? @"WDExpress.cs.12.0\shell\Open\Command" : @"VisualStudio.cs.12.0\shell\Open\Command";
            ...
            {
                string[] content = VS120IDE.GetImportedScripts(projFile);
                ...
                RunPreScripts(scriptFile);
                string newSolution = VS120IDE.CreateProject(content[0], Path.GetDirectoryName(projFile));
                string[] newContent = VS120IDE.GetImportedScripts(Path.ChangeExtension(newSolution, ".csproj"));
